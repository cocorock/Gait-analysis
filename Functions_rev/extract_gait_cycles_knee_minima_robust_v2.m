%% extract_gait_cycles_knee_minima_robust_v2: Detects gait cycles and returns separated data for each leg.
%
% Credits:
...
%   them by analyzing the hip movement.
%   This version separates the output into two structures, one for cycles
%   identified by the right leg and one for the left leg. Each structure
%   contains the data for all four joints (left/right hip/knee) for each cycle.
%
% Input:
...
%
% Output:
%   right_valid_cutted_cycles - struct array: Contains segmented data for all 4 joints for each cycle identified using the right leg.
%   left_valid_cutted_cycles - struct array: Contains segmented data for all 4 joints for each cycle identified using the left leg.

function [right_valid_cutted_cycles, left_valid_cutted_cycles] = extract_gait_cycles_knee_minima_robust_v2(filename, show_debug_plot, enable_printing)
    if nargin < 2
    ...
    right_valid_cutted_cycles = struct('cycle_number', {}, 'start_frame', {}, 'end_frame', {}, 'start_time', {}, 'end_time', {}, 'duration', {}, 'right_hip_flex', {}, 'left_hip_flex', {}, 'right_knee_flex', {}, 'left_knee_flex', {});
    left_valid_cutted_cycles = struct('cycle_number', {}, 'start_frame', {}, 'end_frame', {}, 'start_time', {}, 'end_time', {}, 'duration', {}, 'right_hip_flex', {}, 'left_hip_flex', {}, 'right_knee_flex', {}, 'left_knee_flex', {});
    ...
    for i = 1:length(right_knee_valid)-1
        ...
        % Store all 4 joint angles for the cycle
        right_valid_cutted_cycles(i).cycle_number = i;
        right_valid_cutted_cycles(i).start_frame = start_frame;
        right_valid_cutted_cycles(i).end_frame = end_frame;
        right_valid_cutted_cycles(i).start_time = time(start_frame);
        right_valid_cutted_cycles(i).end_time = time(end_frame);
        right_valid_cutted_cycles(i).duration = time(end_frame) - time(start_frame);
        
        right_valid_cutted_cycles(i).right_hip_flex = right_hip_flex(cycle_indices);
        right_valid_cutted_cycles(i).left_hip_flex = left_hip_flex(cycle_indices);
        right_valid_cutted_cycles(i).right_knee_flex = right_knee_flex(cycle_indices);
        right_valid_cutted_cycles(i).left_knee_flex = left_knee_flex(cycle_indices);
    end
    ...
    for i = 1:length(left_knee_valid)-1
        ...
        % Store all 4 joint angles for the cycle
        left_valid_cutted_cycles(i).cycle_number = i;
        left_valid_cutted_cycles(i).start_frame = start_frame;
        left_valid_cutted_cycles(i).end_frame = end_frame;
        left_valid_cutted_cycles(i).start_time = time(start_frame);
        left_valid_cutted_cycles(i).end_time = time(end_frame);
        left_valid_cutted_cycles(i).duration = time(end_frame) - time(start_frame);

        left_valid_cutted_cycles(i).right_hip_flex = right_hip_flex(cycle_indices);
        left_valid_cutted_cycles(i).left_hip_flex = left_hip_flex(cycle_indices);
        left_valid_cutted_cycles(i).right_knee_flex = right_knee_flex(cycle_indices);
        left_valid_cutted_cycles(i).left_knee_flex = left_knee_flex(cycle_indices);
    end

    % Final summary
    if enable_printing
        fprintf('\n\n=== ROBUST VALIDATION SUMMARY ===\n');
        fprintf('Total validated gait cycles (Right Leg): %d\n', length(right_valid_cutted_cycles));
        fprintf('Total validated gait cycles (Left Leg): %d\n', length(left_valid_cutted_cycles));
    end
