...
function save_processed_data_4D(processed_data, output_dir)
    ...
    if isfield(processed_data, 'right_leg_cycles') && ~isempty(processed_data.right_leg_cycles)
        ...
        right_leg_data_4D = zeros(8, cycles_to_save_right * interp_length);
        ...
        for i = 1:cycles_to_save_right
            ...
            for j = 1:length(joint_fields_filtered)
                right_leg_data_4D(j, start_col:end_col) = current_cycle.(joint_fields_filtered{j});
            end
            
            % Add velocities
            for j = 1:length(joint_fields_velocity)
                right_leg_data_4D(j + 4, start_col:end_col) = current_cycle.(joint_fields_velocity{j});
            end
        end
    ...
    if isfield(processed_data, 'left_leg_cycles') && ~isempty(processed_data.left_leg_cycles)
        ...
        % Initialize the 8xN_samples*N_cycles matrix
        left_leg_data_4D = zeros(8, cycles_to_save_left * interp_length);
        ...
        for i = 1:cycles_to_save_left
            ...
            % Add filtered positions
            for j = 1:length(joint_fields_filtered)
                left_leg_data_4D(j, start_col:end_col) = current_cycle.(joint_fields_filtered{j});
            end
            
            % Add velocities
            for j = 1:length(joint_fields_velocity)
                left_leg_data_4D(j + 4, start_col:end_col) = current_cycle.(joint_fields_velocity{j});
            end
        end
