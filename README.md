...
For users who prefer to work with the data in Python, a script is provided to convert the `.mat` files generated by the MATLAB analysis into a more Python-friendly format (CSV and NumPy array).

### `convert_mat_to_numpy.py`

-   **Purpose**: This script reads a `.mat` file containing gait data, processes it into a structured NumPy array, and saves it as a CSV file.
-   **Input**: `Gait Data/linear_kinematics_gait_cycles_10_samples.mat`
-   **Output**: `Gait Data/linear_kinematics_gait_cycles_10_samples.csv`
...
```bash
python convert_mat_to_numpy.py
```

This will generate a CSV file in the `Gait Data` directory with the same base name as the input `.mat` file. The script also prints the head of the resulting pandas DataFrame to the console.
